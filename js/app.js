...
const MOOD_MESSAGES = {
  idle:        ['Ready to eat, Elizabeth? 🍴','Time to be a good eater! 🌟','You can do it! 🐾'],
  running_lots:['Eating so well, Elizabeth! ⭐','So proud of you! 🌸','Keep going! 🎀'],
  ...
  urgent:      ['Quick quick! ⚡','Last bites! 🏃',"You're almost there! 💨"],
};
const CONFETTI=['🎊','🌟','🎈','👑','🌸','✨','🎀','🍬','🌈','💖','⭐','🦋'];
...
function buildFoodGrid() {
  ...
  CAT_ORDER.forEach(catKey=>{
    ...
    label.className='food-cat-label'; label.textContent=CAT_META[catKey].label;
...
function toggleFood(food,card) {
  ...
  if(idx>-1){ selectedFoods.splice(idx,1); card.classList.remove('selected'); }
  else {
...
function goToTimer() {
  ...
  // Pick random animal for this session
  initAnimal();
  ...
  document.getElementById('animal-wrap').classList.remove('shake','celebrate');
...
function resetTimer() {
  ...
  resumeAnimal(); setAnimalMood('idle');
  PLATE.setTime(totalSeconds,remainingSeconds);
...
function celebrate(type) {
  ...
  pauseAnimal();
  document.getElementById('animal-wrap').classList.remove('shake');
  document.getElementById('animal-wrap').classList.add('celebrate');

  if(type==='done'){
    document.getElementById('celeb-icon').textContent='🎉';
    document.getElementById('celeb-title').textContent='Amazing, Elizabeth!';
    document.getElementById('celeb-msg').textContent=
      'You finished before the plate was empty! '+currentAnimal.emoji+' '+currentAnimal.name+' is so proud! 🏆';
    playFanfare();
  } else {
    document.getElementById('celeb-icon').textContent='⏰';
    document.getElementById('celeb-title').textContent="Time's up!";
    document.getElementById('celeb-msg').textContent=
      currentAnimal.emoji+' '+currentAnimal.name+' says: eat a little faster next time! 🐾💪';
    playTimeout();
  }
